...
@export var boss_threshold: int    = 30
@export var kills_per_point: int   = 5
@export var points_for_boss: int   = 10

@export var mixed_questions: bool = true   # включает смешанный режим
...
func _ready():
	...
	# Устанавливаем режим вопроса (mixed)
	if upgrade_menu and upgrade_menu.has_method("set_question_mode"):
		upgrade_menu.set_question_mode("mixed" if mixed_questions else "math")
...
func _on_boss_killed():
	...
	# Переход на катсцену после третьего уровня (можно на титры)
	get_tree().change_scene_to_file("res://scenes/ending.tscn")  # или создать сцену победы
