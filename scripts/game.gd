...
@export var total_enemies: int     = 20      # всего врагов на уровне
@export var boss_threshold: int    = 20      # убийств до появления босса
...
func _ready():
	_load_scenes()
	_spawn_all_enemies()          # ← вместо таймера
...
func _on_boss_killed():
	...
	# Переход на катсцену (или сразу на уровень 2)
...
func _show_point_notify():
	...
	tw.tween_property(point_notify, "modulate:a", 0.0, 0.4)
	tw.tween_callback(func():
		point_notify.modulate = Color.WHITE
		point_notify.visible = false
	)
...
func _on_exit_button_pressed():
	get_tree().paused = false      # снимаем паузу, если она была
